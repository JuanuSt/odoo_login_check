...
# This script runs every day at 23:58
...
all_login_lines=$(grep -r "${date_today}" "$pg_log_dir" --include=*.log | grep -B1 'FATAL' | grep -B1 'password\|host' | awk '{print $2,$3,$4,$9,$14}' | \
 tr -d '«' | tr -d '»' | sed 's/password/user=/'  | sed 's/^\s*$/+/' | tr -d '\n')
